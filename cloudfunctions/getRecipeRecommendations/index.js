...
exports.main = async (event, context) => {
  ...
  const { scenario = 'single', recipeId, searchKey, filter, page = 1, pageSize = 10, limit } = event

  
  try {
    // === 查询单个菜谱详情（统一从 recipes 集合读取） ===
    if (recipeId) {
    ...
    // === 列表模式：统一走 recipes 集合 ===
    return await fetchRecipesFromDatabase(openid, { scenario, searchKey, filter, page, pageSize, limit })
...
async function fetchRecipesFromDatabase(openid, options) {
...
//展示所有菜谱
    ...
    // quick/hearty/light 是前端旧筛选项，后端不做硬过滤，避免无数据
    ...
    console.error('❌ [Database] 菜谱获取异常:', e.message)
    return {
      success: true,
      recipes: [],
      total: 0,
