...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // === Step 2: 复用 getUserFoods 云函数获取食材（含共享组逻辑）===
    const foodsRpc = await cloud.callFunction({
      name: 'getUserFoods',
      data: { openid }
    })
    const foods = (foodsRpc.result && foodsRpc.result.success) ? foodsRpc.result.data : []
    ...
    for (const ing of ingredients) {
      ...
      if (matchedFood) {
        // 调用 manageFoodItem 统一处理扣减（含共享组权限校验）
        const rpc = await cloud.callFunction({
          name: 'manageFoodItem',
          data: {
            action: 'consume',
            itemId: matchedFood._id,
            amount: ing.amount || 1,
            openid,   // ★ 显式传递 openid
          }
        })
        const result = (rpc.result && rpc.result.success) ? rpc.result : null
